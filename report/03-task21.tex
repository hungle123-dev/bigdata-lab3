...
\textbf{Dữ liệu đầu vào:} nhóm tiền xử lý \texttt{Amazon Sale Report.csv} (128.975 dòng, raw) bằng \texttt{clean.ipynb} $\to$ \texttt{data/asr.csv}: chuẩn hoá \texttt{ship-state} (gộp cách viết trùng \texttt{NEW DELHI}$\to$\texttt{DELHI}, \texttt{ORISSA}$\to$\texttt{ODISHA}, \texttt{RJ}$\to$\texttt{RAJASTHAN}\ldots), loại \textbf{33 dòng thiếu state} và \textbf{1 dòng state không hợp lệ \texttt{APO}} --- tổng \textbf{34 dòng} bị loại $\to$ \textbf{128.941 dòng, 36 bang}. State normalization làm ở tiền xử lý, không lặp lại trong Spark.

\textbf{Lưu ý output khác slide/reference:} slide tham khảo tính trên dataset \emph{raw} (chưa làm sạch), nhóm dùng \emph{canonical cleaned input} nên một số con số trung gian (số bang, số dòng bought\ldots) lệch nhẹ. Đây là hệ quả của tiền xử lý đã thống nhất, không phải sai khác logic.
...
B & Ngưỡng bang & \texttt{Merchant AND Courier=Shipped}, \texttt{groupBy(state).avg} & 31.871 dòng $\to$ \textbf{36} bang \\
...
\begin{itemize}
  ...
  \item Chuẩn hoá bang làm ở tiền xử lý (\texttt{clean.ipynb}, 36 bang). Task 2-1 chỉ \texttt{upper(trim)} thành phố. Trong tập ứng viên: 1.639 cách viết \texttt{ship-city} thô $\to$ \textbf{1.434} city chuẩn hoá. Tập ứng viên có \textbf{6.906} đơn, mọi đơn đều có \texttt{ship-city} (candidate city-null = 0, vì đơn thiếu state đã bị loại ở tiền xử lý) $\to$ mẫu số \textbf{6.906}.
