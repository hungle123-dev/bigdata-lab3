...
D & Gộp thành phố & \texttt{groupBy(city)}, \% đơn thoả & \textbf{1.435} city \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item Chuẩn hoá \texttt{upper(trim(...))} bang \& thành phố (69 $\to$ 47 cách viết bang).
\end{itemize}

\subsection*{(2) Kết quả = 0\% mọi thành phố — có chứng minh}
Tử số bằng 0 ở mọi thành phố. Đây \emph{là} đáp án đúng, không phải lỗi. Chứng minh bằng cách nới dần điều kiện:
\begin{center}
\begin{tabular}{lrrr}
\hline
...
\quad Sort node & 1 & 7 \\
(c) WholeStageCodegen (max id) & 9 & 15 \\
\hline
\end{tabular}
\end{center}

\textbf{(a)} 3 bảng đi join đều tí hon (185 mã hợp lệ, bảng đếm mã/đơn, 40 ngưỡng bang) $<$ 10 MB $\to$ Spark broadcast cả 3, khỏi shuffle bảng đơn. Ép \texttt{threshold=-1} buộc SortMergeJoin: mỗi join thêm 1 Exchange + 2 Sort hai phía.
...
\textbf{(c)} Số stage đọc ở Spark UI tab Jobs (mỗi Exchange chia một ranh giới stage).
