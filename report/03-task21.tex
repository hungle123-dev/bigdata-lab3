...
B & Ngưỡng bang & \texttt{Merchant AND Courier=Shipped}, \texttt{groupBy(state).avg} & 31.881 dòng $\to$ \textbf{40} bang \\
C & Tập đơn + đếm mã hợp lệ & lọc ứng viên; LEFT join A; LEFT join B & \textbf{6.909} đơn \\
...
\begin{itemize}
  ...
  \item Chuẩn hoá \texttt{upper(trim(...))} bang \& thành phố (69 $\to$ 47 cách viết bang). 33 đơn thiếu \texttt{ship-city} bị loại (``each city'' không tính null).
...
Tử số bằng 0 ở mọi thành phố (1.434 city sau khi loại 33 đơn thiếu \texttt{ship-city}). Đây \emph{là} đáp án đúng, không phải lỗi. Chứng minh bằng cách nới dần điều kiện:
\begin{center}
\begin{tabular}{lrrr}
\hline
Cách hiểu ``cancelled'' & Số đơn & Max mã/đơn & $\ge 3$ mã \\
\hline
Status chứa cancelled + Standard & 6.909 & 0 & \textbf{0} \\
...
\end{tabular}
\end{center}
Toàn bộ 6.909 đơn Cancelled+Standard có \texttt{promotion-ids} rỗng; nới hết mức thì max chỉ 2 mã/đơn $\to$ điều kiện $\ge 3$ không bao giờ chạm được. \textbf{Không} hạ ngưỡng để ``ra số đẹp''. File xuất giữ cột chứng minh: \texttt{city, total\_orders, qualified\_orders, pct\_cancelled}.
...
\quad Sort node & 1 & 7 \\
(c) Số stage (SparkListener, action \texttt{write}) & 9 & --- \\
\hline
\end{tabular}
\end{center}

\textbf{(a)} 3 bảng đi join đều tí hon (185 mã hợp lệ, bảng đếm mã/đơn, 40 ngưỡng bang) $<$ 10 MB $\to$ Spark broadcast cả 3, khỏi shuffle bảng đơn. Ép \texttt{threshold=-1} buộc SortMergeJoin: mỗi join thêm 1 Exchange + 2 Sort hai phía (Sort: $1 \to 7$).

\textbf{(b)} Mặc định: 5 shuffle Exchange = 4 \texttt{groupBy} (mã KM, row\_id, state, city) + 1 \texttt{rangepartitioning} của \texttt{orderBy} cuối. Broadcast không tạo shuffle bảng lớn nên chỉ có 3 BroadcastExchange bảng nhỏ. Tắt broadcast: 8 Exchange (3 join giờ phải shuffle cả hai phía) và 7 Sort.

\textbf{(c)} Đo bằng \texttt{SparkListener.onStageCompleted} bao quanh action \texttt{write} (đã cài trong \texttt{Task21.scala}): \textbf{9 stage} (AQE bật, broadcast bật). Số này in ra console mỗi lần chạy (\texttt{writeStages=9}). Với AQE, stage có thể coalesce động — đọc thêm ở Spark UI tab Jobs (\texttt{localhost:4040}).
