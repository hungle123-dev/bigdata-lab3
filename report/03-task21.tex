...
D & Gộp thành phố & \texttt{groupBy(city)}, \% đơn thoả & \textbf{1.434} city (6.906 đơn) \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item Chuẩn hoá \texttt{upper(trim(...))} bang \& thành phố (69 $\to$ 47 cách viết bang). Trong tập ứng viên: 1.639 cách viết \texttt{ship-city} thô $\to$ \textbf{1.434} city chuẩn hoá. 3/6.909 đơn thiếu \texttt{ship-city} bị loại trước \texttt{groupBy} (``each city'' không tính null) $\to$ mẫu số cuối \textbf{6.906} đơn.
...
Tử số bằng 0 ở mọi thành phố. Kết quả: 1.434 city, mẫu số 6.906 đơn (6.909 ứng viên trừ 3 đơn thiếu \texttt{ship-city}). Đây \emph{là} đáp án đúng, không phải lỗi. Chứng minh bằng cách nới dần điều kiện:
\begin{center}
\begin{tabular}{lrrr}
\hline
